            ...
            data: (phaseDefs) {
              return ref.watch(cycle_providers.currentCycleProvider).when(
                data: (cycle) {
                  ...
                  final fastingRec = ref.watch(
                    phase_providers.userPhaseRecommendationsByDayProvider(cycleDay),
                  );
                  return fastingRec.when(
                    data: (allRecommendations) {
                      // Filter for Fasting category
                      final recommendations = allRecommendations['Fasting'] ?? [];
                              ...
                              children: [
                                Row(
                                  children: [
                                    ...
                                    Expanded(
                                      child: Text(
                                        '${log.startTime.toString().split('.')[0].split(':').take(2).join(':')} - ${log.endTime.toString().split('.')[0].split(':').take(2).join(':')}',
